import rasterio
import os
from geopy.distance import distance # or your preferred geodetic library
...
class geoInfo:
    ...
    def create_2D_dictionary(self, link: P2PLink, ds_flag: DSFlags):
        ...
        for s in segments:             
            ...
            box = s['bbox']        
            res.append(du.extract_and_profile(src, *box, tx, rx, band_index=self.band_index))    
            segs.append((tx,rx))
    ...
    def show_fresnel_profiles(self, link: P2PLink, v_h = 0, v_v = -0.6, **kwargs):
        ...
        du.plot_data_tuples(res_dtm, res_dsm, res_lulc, res_fresnel, labels=['dtm', 'dsm', 'lulc', 'fresnel'], figsize=(12.5,6) ) 
    ...
    def show_lulc_fresnel_zone( self, link : P2PLink, **kwargs, ):
        ...
        du.show_lulc_fresnel_from_srcs(
            srcs,
    ...
    def create_b64_map(self, tx, rx, radius_m =10, ds_flag : DSFlags = DSFlags.DTM):
        from cisei_lib.core.plot.geo_plot import show_tiff             

        west, south, east, north = du.compute_corridor_bbox(*tx, *rx, radius_m)

        self.select_dataset(ds_flag, [tx, rx] )        
        src = self.get_cached_raster(ds_flag, *tx)
        ...
        return show_tiff(src, window, marks=marks, legend=legend, colors=colors)
    ...
    def create_b64_link(self, link: P2PLink, v_h = 0, mode='linear_max', **kwargs):
        ...
        return plot_profile(
            ...
            los=los,
            dpi=kwargs.get('dpi'),
            size=(kwargs.get('w'), kwargs.get('h'))
        )
