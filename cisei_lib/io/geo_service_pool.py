...
'''
First request for A–B:
    not cached
    not in in_flight
    create task
    store task in in_flight[A–B]

Second request for A–B:
    not cached yet
    finds existing task in in_flight[A–B]
    awaits that same task
After completion:
    result stored in cache
    A–B removed from in_flight
...
class AsyncGeoServicePool:
    ...
    async def start(self) -> AsyncGeoServicePool:
        ...
        try:
            # Sequential registration avoids simultaneous registrations.
    ...
    async def close(self) -> None:
        ...
        # Allow already-submitted extractions to finish.
    ...
    async def get_request(
        ...
        if task is None:
            # No await occurs between checking and inserting the task.
            # Therefore, this section is atomic in the event-loop thread.
            task = asyncio.create_task(
                self._calculate_and_cache(key, request)
            )
            self._in_flight[key] = task

        # One caller being cancelled must not cancel the shared calculation.
    ...
    def _extract_blocking(
        ...
        # Do not use client.link(..., prepare_link=True) here because that
        # method discards the set_link response. Validate each stage.
    ...
    def _validate_result(
        ...
        if result.kind in {
            ...
            raise GeoServiceError(
                f"{operation} failed: {result.data}"
            )
    ...
    def _freeze(cls, value: Any) -> Any:
        ...
        try:
            ...
            raise TypeError(
                f"Cannot use {type(value).__name__} "
                "as part of the feature cache key"
            ) from exc
