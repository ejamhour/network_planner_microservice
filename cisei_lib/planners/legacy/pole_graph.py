from cisei_lib.io.geo_service_client import GeoServiceClient
...
class PoleGraph:
    def __init__(self, **kwargs):
        ...
        geo_url = kwargs.get("geo_url")

        self.geo = GeoServiceClient(base_url=geo_url)
        self.geo.register()
    ...
    def edge_metric(self, src_pos, dst_pos, src_ha, dst_ha) -> dict | None:
        ...
        self.geo.link(src_pos, dst_pos, tx_ha=src_ha, rx_ha=dst_ha, freq_mhz=self.context['freq_mhz'])
        res = self.geo.link_features()
        
        if res.data['status'] == 'error':
            self._debug(f'failed to compute the features for the edge {src_pos} {dst_pos}')            
            return None

        features = res.data['data']

        metric = self.metric_function(features)
                
        features['obs_pos'] = self._top_peak_coordinates(src_pos, dst_pos, features)
    ...
    def run(self, src_node, dst_node):
...
# Private methods
    ...
    def _get_poles_by_dual_cone(self, s_label, d_label):
        ...
        result = self.geo.point_samples(
            points,
            ds_string="DTM",
            nodata_value=0,
        )

        poles_in_radius["elevation"] = result.data['data']
